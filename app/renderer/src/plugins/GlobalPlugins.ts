/**
 * bridge. GlobalPluginLoader
 * Loads v2 global plugins
 *
...
let GLOBAL_PLUGIN_FOLDERS: string[]
let GLOBAL_PLUGIN_DATA: any[] = []
...
export default class GlobalPluginLoader {
	...
	static async loadPlugins() {
		...
		let PLUGIN_ZIPS = GLOBAL_PLUGIN_FOLDERS.filter(
			p => path.extname(p) === '.zip'
		)
		GLOBAL_PLUGIN_FOLDERS = GLOBAL_PLUGIN_FOLDERS.filter(
			p => path.extname(p) !== '.zip'
		)
		//Initialize PLUGIN_DATA with UI_DATA of BridgeCore
		GLOBAL_PLUGIN_DATA = [UI_DATA]
		await Promise.all(
	...
	static async loadPlugin(plugin_folder: string, unloaded_plugins: string[]) {
		...
		if ((await fs.lstat(pluginPath)).isFile()) {
			if (path.extname(pluginPath) === '.js') {
				 ...
				 */
				if (
					GLOBAL_PLUGIN_FOLDERS.includes(
						path.basename(plugin_folder, '.zip')
					)
				) {
					GLOBAL_PLUGIN_FOLDERS = GLOBAL_PLUGIN_FOLDERS.filter(
			...
			GLOBAL_PLUGIN_DATA.push(manifest)
