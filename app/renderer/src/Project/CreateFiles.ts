...
export default class CreateFiles {
	...
	static async load() {
		this.create = JSON.parse(
			(
				await fs.readFile(
					path.join(__static, 'data/create_project.json')
				)
			).toString()
		)
	}
	...
	static async createPack(
		...
		await Promise.all(
			pack.folders.map(async f =>
				fs.mkdir(path.join(base_path, f), { recursive: true })
			)
		)
		...
		for (let data_name in pack.files) {
			if (typeof pack.files[data_name] === 'string') {
				proms.push(
					fs.copyFile(
						path.join(__static, 'data', data_name),
						path.join(base_path, pack.files[data_name] as string)
					)
				)
			} else {
				proms.push(
					this.parseReplacer(
						data_name,
						base_path,
						pack.files[data_name] as Replacer,
						env
					)
				)
			}
	...
	static async parseReplacer(
		...
		let file_content = (
			await fs.readFile(path.join(__static, 'data', p))
		).toString()
		...
		for (let repl in with_repl) {
			...
			file_content = file_content.replace(
				new RegExp(escapeRegExpStr(repl), 'g'),
				statement || with_repl[repl]
			)
		}
