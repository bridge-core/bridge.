...
export class JSONMask {
	...
	set(
		...
		if (this.data[channel] === undefined)
			return (this.data[channel] = mask_data)
		if (merge_all)
			return (this.data[channel] = detachMerge(
				this.data[channel],
				mask_data
			))
		if (merge_arrays.length === 0)
			return (this.data[channel] = detachObj(
				this.data[channel],
				mask_data
			))
		return (this.data[channel] = maskChannelMerge(
			this.data[channel],
			mask_data,
			merge_arrays
		))
	...
	all(filter?: (layer_name: string) => boolean) {
		...
		for (let c in this.data) {
			if (
				this.data[c] !== undefined &&
				(typeof filter !== 'function' || filter(c))
			)
...
export class JSONFileMasks {
	...
	static async applyOnData(
		file_path: string,
		data: any,
		filter: (layer_name: string) => boolean,
		overwrite_arrays?: string[]
	) {
		return maskMerge(
			[...(await this.get(file_path)).all(filter), data],
			overwrite_arrays
		)
	}

	static async generateFromMask(
		file_path: string,
		overwrite_arrays?: string[]
	) {
		await fs.mkdir(path.dirname(file_path), { recursive: true })
		return writeJSON(
			file_path,
			maskMerge((await this.get(file_path)).all(), overwrite_arrays),
			true
		)
	}
}
