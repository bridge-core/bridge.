...
export default class SearchListView extends ContentWindow {
	constructor(search_filter, from = 0) {
		super(
			{
				display_name: search_filter
					? `Debug Log | Search: ${search_filter.toUpperCase()}`
					: 'Debug Log',
				options: {
					is_persistent: false,
		...
		processedDebugLog().then(logs => {
			if (search_filter)
				logs = logs.filter(
					({ tags, error }) =>
						tags.includes(search_filter) ||
						error.includes(search_filter)
				)
			...
			this.actions = [
				{ type: 'space' },
				{
					text: `${from / PAGE_SIZE + 1}/${Math.ceil(
