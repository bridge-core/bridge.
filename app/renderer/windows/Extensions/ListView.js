...
export default class ListView extends ContentWindow {
	constructor(tag_filter) {
		super(
			{
				display_name: tag_filter
					? `Extensions | Filter: ${tag_filter}`
					: 'Extensions',
				options: {
					is_persistent: false,
		...
		Session.open().then(data => {
			this.content = BASE.concat(
				data
					.filter(({ tags }) =>
						tag_filter ? tags.includes(tag_filter) : true
					)
					.map(plugin => [
			...
			if (this.content.length === 4) {
				this.content = this.content.concat([
					{
						text: `No matching results found for the filter "${tag_filter}"`,
					},
				])
			}
