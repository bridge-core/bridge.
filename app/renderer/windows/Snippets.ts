...
async function assureLoadedSnippets() {
	if (SNIPPETS === undefined) SNIPPETS = await FileType.getSnippets()
}
...
async function convArr() {
	...
	return snippets
		.map((s, i) =>
			s.is_hidden ? undefined : { value: i, text: s.display_name }
		)
...
async function insertSnippet(
	...
	if (c instanceof JSONTree) {
		if (
			Store.state.Settings.snippet_scope === 'Default' ||
			templ.force_default_scope ||
			force_default_scope
		) {
			c.get('global').buildFromObject(
				expandTemplateData(templ.data, templ.data_path),
				true,
				true,
				true
			)
		} else {
			TabSystem.getCurrentNavObj().buildFromObject(
				templ.data,
				true,
				true,
				true
...
class SnippetWindow extends ContentWindow {
	...
	async showWin() {
		this.content[2].options = (await convArr()).sort((a, b) =>
