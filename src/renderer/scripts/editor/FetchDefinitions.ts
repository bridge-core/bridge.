...
export default class FetchDefinitions {
	...
	static async getAll(file_type: string, cache_key: string) {
		...
		for (let p in c) res[p] = c[p][cache_key]
		return res
	}
	...
	static async fetchSingle(
		...
		for (let d of fetch_defs) {
			...
			for (let file in c[file_type]) {
				...
				if (
					c[file_type][file][d] &&
					c[file_type][file][d].includes(fetch_search)
				) {
					let file_path
					try {
						//LOAD FILE PATH
						if (file.startsWith('BP')) {
							...
							console.error(
								'Unexpected cache file start: ' + file
							)
						}
					} catch (e) {
						/*CACHE WAS PREVIOUSLY BROKEN - PREVENTS ERROR WITH PATH COMPOSITION*/
					}
		...
		for (let file_type in c) {
			...
			for (let file in c[file_type]) {
				...
				for (let cache_key in c[file_type][file]) {
					//LOOKUP CACHE KEY
					if (
						c[file_type][file][cache_key] &&
						c[file_type][file][cache_key].includes(fetch_search)
					) {
						let file_path
						try {
							//LOAD FILE PATH
