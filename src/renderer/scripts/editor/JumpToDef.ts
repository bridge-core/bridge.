...
export default class JumpToDefinition {
    ...
    static cache: JTDCache = { file_types: [], fetch_search: undefined, fetch_all: true, result: undefined };
    ...
    static cacheValid(file_types: string[], fetch_search: string, fetch_all?: boolean) {
    ...
    static async fetch(file_types: string[], fetch_search: string, fetch_all=true) {
    ...
    static async fetchSingle(file_type: string, fetch_search: string, fetch_all?: boolean) {
        return await FetchDefinitions.fetchSingle(file_type, this.data[file_type], fetch_search, fetch_all);
    }

    static fetchSync(file_types: string[], fetch_search: string, fetch_all=true) {
        if(this.cacheValid(file_types, fetch_search, fetch_all)) return this.useCache(fetch_all);
    ...
    static fetchSingleSync(file_type: string, fetch_search: string, fetch_all?: boolean) {
