...
export default class FileSystem {
    ...
    static async save(file_path: string, content: string | Buffer, update=false, open=false) {
    ...
    static async openFile(file_path: string) {
        let file: Buffer;
        try { file = await fs.readFile(file_path); } catch { return; }
        let cache; 
        try { cache = await OmegaCache.load(file_path); } catch { return this.loadFromDisk(file_path, file); }
    ...
    static async openDir(file_path: string) {
        ...
        await Promise.all(
            dirents.map(async dirent => {
                if(dirent.isFile()) await this.openFile(path.join(file_path, dirent.name));
