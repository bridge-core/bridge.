...
const mutations = {
	setSidebarMenu(state, new_menu_state) {
		...
		Bridge.trigger(
			'bridge:openedSidebar',
			opened ? opened.id || opened.menu_type : null,
			true
		)
	},
	...
	addPluginSidebar(state, new_sidebar) {
		let i = 0
		while (
			i < state.plugin_items.length &&
			state.plugin_items[i].id != new_sidebar.id
		) {
			i++
		}
	...
	updatePluginSidebar(state, new_sidebar) {
		...
		if (i < state.plugin_items.length) {
			let tmp = detachObj(state.plugin_items[i], {
				...new_sidebar,
				is_plugin: true,
			})
	...
	removePluginSidebar(state, id) {
		...
		while (
			i < state.plugin_items.length &&
			state.plugin_items[i].id != id
		) {
			i++
		}

		if (i < state.plugin_items.length) state.plugin_items.splice(i, 1)
		if (state.menu_state - 1 == i + state.items.length)
