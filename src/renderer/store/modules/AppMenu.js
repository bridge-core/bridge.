	...
	edit: {
		...
		elements: [
			...
			},
			{
				...
				shortcut: 'Ctrl + Z',
				action: () => {
					if (!TabSystem.getHistory().undo())
						EventBus.trigger('cmUndo')
				},
			},
			{
				title: 'Redo',
				shortcut: 'Ctrl + Y',
				action: () => {
					if (!TabSystem.getHistory().redo())
						EventBus.trigger('cmRedo')
				},
			},
			{
				type: 'divider',
	...
	help: {
		...
		elements: [
			...
			},
			{
				...
				action: () =>
					shell.openExternal(
						'https://github.com/solvedDev/bridge./releases/latest'
					),
			},
			{
				title: 'Bug Report',
