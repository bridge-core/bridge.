...
export default {
	...
	computed: {
		...
		selected_nodes() {
			return this.selection.reduce(
				(prev, curr) => prev.concat(curr.nodes),
				[]
			)
		},
	},
	methods: {
		search() {
			if (this.search_all) {
				...
				if (c instanceof JSONTree) {
					...
					nodes.forEach(
						c => (c.mark_color = 'rgba(119, 119, 119, 0.3)')
					)
		...
		reset() {
			this.selection.forEach(({ nodes }) =>
				nodes.forEach(n => (n.mark_color = undefined))
			)
			this.selection = []
		},
	},
