<template>
    <div>
        <v-toolbar color="expanded_sidebar" flat height="30px">
            <v-tooltip color="tooltip" bottom>
                <template v-slot:activator="{ on }">
                    <v-btn icon text @click.stop="openInstallExtensionWindow" v-on="on" small class="toolbar-button">
        ...
        <div class="container" :style="`height: ${plugin_height}px;`">
            ...
            <v-card color="expanded_sidebar" v-for="(plugin, i) in plugins" style="margin-bottom: 8px;" :key="`${plugin.id}.${i}`">
