<template>
	...
	>
		...
		>
				...
				<v-list
					...
					:style="
                        ...
                        border-right: 1px solid rgba(${
							is_dark_mode ? '255, 255, 255' : '0, 0, 0'
						}, 0.12) !important;
                        position: absolute;
                        height: ${is_fullscreen ? maxHeight : height}px;
                        padding: 8px 0;
			...
			<v-card-text
				v-else-if="win.content"
				class="main-content"
				:style="`height: ${is_fullscreen ? maxHeight : height}px;`"
			>
...
export default {
	...
	computed: {
		is_window_visible: {
			...
			set(val) {
				this.$store.commit('setWindowIsVisible', {
					id: this.win.id,
					val,
				})
			},
		...
		has_close_button() {
			return (
				!this.win.options ||
				this.win.options.is_closable == undefined ||
				this.win.options.is_closable
			)
		},
		has_maximize_button() {
			return (
				!this.win.options ||
				this.win.options.is_maximizable == undefined ||
				this.win.options.is_maximizable
			)
		},
		has_toolbar() {
			return (
				!this.win.options ||
				this.win.options.is_frameless == undefined ||
				!this.win.options.is_frameless
			)
		},
		has_no_padding() {
		...
		blur_background() {
			return (
				!this.win.options ||
				this.win.options.blurs_background === undefined ||
				this.win.options.blurs_background
			)
		},
		is_persistent() {
			return (
				!this.win.options ||
				this.win.options.is_persistent == undefined ||
				this.win.options.is_persistent
			)
		},
		is_dark_mode() {
		...
		width() {
			if (!this.win.options || this.win.options.width == undefined)
				return 500
			return this.win.options.width
		},
		height() {
			if (!this.win.options || this.win.options.height == undefined)
				return 500
			return this.win.options.height
		},
		maxWidth() {
			if (!this.win.options || this.win.options.maxWidth == undefined)
				return '90%'
			return this.win.options.maxWidth
		},
		maxHeight() {
			if (!this.win.options || this.win.options.maxHeight == undefined)
				return this.window_height * 0.8
	...
	watch: {
		is_window_visible(new_val) {
			if (!new_val && typeof this.win.onClose === 'function')
				this.win.onClose()
			if (new_val && typeof this.win.onOpen === 'function')
				this.win.onOpen()
		},
	},
	methods: {
	...
	mounted() {
		let drag_region
		if (this.$refs.drag_region != undefined)
			drag_region = this.$refs.drag_region.$el
		...
		if (drag_region && this.win.options && this.win.options.is_draggable) {
			...
			drag_region.addEventListener('mousedown', ev => {
				if (ev.target.tagName == 'I' || ev.target.tagName == 'BUTTON')
					return
				...
				let mousemove = ev => {
					...
					movable.style.left =
						Number(movable.style.left.replace('px', '')) -
						off_x +
						'px'
					movable.style.top =
						Number(movable.style.top.replace('px', '')) -
						off_y +
						'px'
